    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.akreditasi-internasionals.update", [$akreditasiInternasional->id]) }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('fakultas') ? 'is-invalid' : '' }}" name="fakultas_id" id="fakultas_id">
                    @foreach($fakultas as $id => $entry)
                        <option value="{{ $id }}" {{ (old('fakultas_id') ? old('fakultas_id') : $akreditasiInternasional->fakultas->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('prodi') ? 'is-invalid' : '' }}" name="prodi_id" id="prodi_id">
                    @foreach($prodis as $id => $entry)
                        <option value="{{ $id }}" {{ (old('prodi_id') ? old('prodi_id') : $akreditasiInternasional->prodi->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
                @if($errors->has('prodi'))
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('jenjang') ? 'is-invalid' : '' }}" name="jenjang_id" id="jenjang_id">
                    @foreach($jenjangs as $id => $entry)
                        <option value="{{ $id }}" {{ (old('jenjang_id') ? old('jenjang_id') : $akreditasiInternasional->jenjang->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('lembaga') ? 'is-invalid' : '' }}" name="lembaga_id" id="lembaga_id">
                    @foreach($lembagas as $id => $entry)
                        <option value="{{ $id }}" {{ (old('lembaga_id') ? old('lembaga_id') : $akreditasiInternasional->lembaga->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('no_sk') ? 'is-invalid' : '' }}" type="text" name="no_sk" id="no_sk" value="{{ old('no_sk', $akreditasiInternasional->no_sk) }}">
                @if($errors->has('no_sk'))
            ...
            <div class="form-group">
                ...
                <input class="form-control date {{ $errors->has('tgl_sk') ? 'is-invalid' : '' }}" type="text" name="tgl_sk" id="tgl_sk" value="{{ old('tgl_sk', $akreditasiInternasional->tgl_sk) }}">
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('tahun_expired') ? 'is-invalid' : '' }}" type="number" name="tahun_expired" id="tahun_expired" value="{{ old('tahun_expired', $akreditasiInternasional->tahun_expired) }}" step="1">
            ...
            <div class="form-group">
                ...
                <select class="form-control {{ $errors->has('peringkat') ? 'is-invalid' : '' }}" name="peringkat" id="peringkat">
                    ...
                    @foreach(App\Models\AkreditasiInternasional::PERINGKAT_SELECT as $key => $label)
                        <option value="{{ $key }}" {{ old('peringkat', $akreditasiInternasional->peringkat) === (string) $key ? 'selected' : '' }}>{{ $label }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('nilai') ? 'is-invalid' : '' }}" type="number" name="nilai" id="nilai" value="{{ old('nilai', $akreditasiInternasional->nilai) }}" step="1">
                @if($errors->has('nilai'))
            ...
            <div class="form-group">
                <div class="form-check {{ $errors->has('diakui_dikti') ? 'is-invalid' : '' }}">
                    ...
                    <input class="form-check-input" type="checkbox" name="diakui_dikti" id="diakui_dikti" value="1" {{ $akreditasiInternasional->diakui_dikti || old('diakui_dikti', 0) === 1 ? 'checked' : '' }}>
            ...
            <div class="form-group">
                ...
                <textarea class="form-control {{ $errors->has('note') ? 'is-invalid' : '' }}" name="note" id="note">{{ old('note', $akreditasiInternasional->note) }}</textarea>
                @if($errors->has('note'))
